      ...
      <main>
        <section className="relative overflow-hidden border-b border-slate-200 bg-white">
          <div className="absolute inset-0 opacity-60">
            <div className="absolute left-[8%] top-20 h-48 w-48 rounded-full border border-teal-100" />
            <div className="absolute right-[12%] top-28 h-64 w-64 rounded-full border border-blue-100" />
            <div className="absolute bottom-12 left-[35%] h-32 w-32 rounded-full border border-violet-100" />
          </div>

          <div className="relative mx-auto grid max-w-7xl gap-10 px-6 py-16 lg:grid-cols-[1.15fr_0.85fr] lg:py-20">
            ...
            <div id="auth" className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <p className="mt-2 text-sm text-slate-600">
                新用户默认获得 3 篇文档嵌入与 50 次问答试用额度，后续可在设置页替换为自己的 API Key。
              </p>
              ...
              <div className="mt-5 rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-900">
                ...
                登录后可在设置页添加自己的 LLM 与 embedding API Key，系统将优先使用个人 Key。
              </div>
            </div>
          </div>
        </section>
        ...
        <section id="features" className="mx-auto max-w-7xl px-6 py-14">
          <div className="mb-8 max-w-2xl">
            ...
            <p className="mt-3 text-sm leading-7 text-slate-600">
              化工实验事实通常由多个变量共同约束。HyperChE 将这类多元关系作为完整超边保留，减少普通图拆边带来的上下文碎片化。
            </p>
          </div>
          <div className="grid gap-4 md:grid-cols-3">
            <FeatureCard icon={<Beaker size={20} />} title="体系—材料—条件—指标" desc="把实验体系、材料组成、操作条件和性能指标作为同一事实单元召回。" />
            <FeatureCard icon={<Atom size={20} />} title="机理证据链" desc="关联活性物种、表征证据、降解路径和性能变化，支持机制型问答。" />
            <FeatureCard icon={<Database size={20} />} title="领域可迁移" desc="通过实体类型、关系类型和提示词配置，扩展到液流电池、PFAS 等化工子领域。" />
          </div>
        </section>
        ...
        <section id="comparison" className="mx-auto max-w-7xl px-6 py-14">
          <h2 className="text-2xl font-semibold text-slate-950">从文本相似到高阶事实召回</h2>
          <div className="mt-6 grid gap-4 lg:grid-cols-3">
            <ComparisonColumn title="Vector RAG" accent="bg-slate-400" items={['召回语义相似文本', '难以做变量共同约束', '回答依赖片段拼接']} />
            <ComparisonColumn title="Graph RAG" accent="bg-blue-600" items={['实体—关系—实体', '多变量事实被拆边', '同一实验条件容易丢失']} />
            <ComparisonColumn title="Hypergraph RAG" accent="bg-violet-600" items={['一条超边连接多个实体', '保留完整实验事实单元', '适合条件组合与机理链条问题']} />
          </div>
          <div className="mt-8 rounded-lg border border-teal-200 bg-teal-50 p-5 text-sm leading-7 text-teal-950">
            ...
            平台提供有限试用额度，适合快速验证文献问答效果。正式批量嵌入时，建议在工作台设置页配置自己的 API Key，以获得更稳定的调用额度。
          </div>
        </section>
      </main>
    </div>
